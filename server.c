...
int main(int argc,char** argv){
    ...
    do { 
        ...
        if (nsid==-1){
            ...
            tmi = localtime(&intps);
            bzero(filename,256);
            sprintf(filename,"clt_%d_%d_%d_%d_%d_%d.jpg",tmi->tm_mday,tmi->tm_mon+1,1900+tmi->tm_year,tmi->tm_hour,tmi->tm_min,tmi->tm_sec);
        ...
        printf("receved a file (%ld) Bytes from %s:%d\n", count, dst, clt_port);
    } while(sfd > 0);
...
int create_server_socket (int port){
   ...
   optval2 = 1;
   setsockopt(sfd, SOL_IP, IP_TRANSPARENT, 
	     (const void *)&optval2 , sizeof(int));
